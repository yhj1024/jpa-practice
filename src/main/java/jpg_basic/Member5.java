...
@Entity
@NamedQuery(
        name = "Member5.findByUsername", // name에 Member 안 붙여도 되는데 관례상 이렇게 많이 사용함
        query = "select m from Member5 m where m.username = :username"
        //query = "select m from Member51asdf m where m.username = :username" // 쿼리에 문제가 있으면 실행 시점에 오류가 발생한다.
        // 가장 좋은 오류는 빌드 타임이고, 가장 안 좋은건 유저가 선택 했을때이다
        // 얘도 런타임이긴 한데 애플리케이션 실행 시점에 오류가 나니까 괜찮은 시점
        // xml 에도 추가 가능하고, xml 이 우선 순위를 가짐
        )
